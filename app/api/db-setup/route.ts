import { NextRequest, NextResponse } from "next/server";

const PROJECT_REF = "qceokprcnnizjzvqxdwa";

async function runSQL(pat: string, query: string) {
  const res = await fetch(
    `https://api.supabase.com/v1/projects/${PROJECT_REF}/database/query`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${pat}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ query }),
    }
  );
  const data = await res.json();
  return { ok: res.ok, data };
}

const SQL_STEPS = [
  {
    label: "visitors.notes カラム追加",
    sql: "ALTER TABLE visitors ADD COLUMN IF NOT EXISTS notes TEXT;",
  },
  {
    label: "visitor_images テーブル作成",
    sql: `CREATE TABLE IF NOT EXISTS visitor_images (
      id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      visitor_id   UUID NOT NULL REFERENCES visitors(id) ON DELETE CASCADE,
      storage_path TEXT NOT NULL,
      filename     TEXT NOT NULL,
      created_at   TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );`,
  },
  {
    label: "visitor_images RLS 有効化",
    sql: "ALTER TABLE visitor_images ENABLE ROW LEVEL SECURITY;",
  },
  {
    label: "visitor_images ポリシー作成",
    sql: `DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='visitor_images' AND policyname='public_all') THEN
        CREATE POLICY "public_all" ON visitor_images FOR ALL USING (true) WITH CHECK (true);
      END IF;
    END $$;`,
  },
  {
    label: "comments テーブル作成",
    sql: `CREATE TABLE IF NOT EXISTS comments (
      id         UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      username   TEXT NOT NULL,
      message    TEXT NOT NULL,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
    );`,
  },
  {
    label: "comments RLS 有効化",
    sql: "ALTER TABLE comments ENABLE ROW LEVEL SECURITY;",
  },
  {
    label: "comments ポリシー作成",
    sql: `DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='comments' AND policyname='public_all') THEN
        CREATE POLICY "public_all" ON comments FOR ALL USING (true) WITH CHECK (true);
      END IF;
    END $$;`,
  },
  {
    label: "comments seen_by カラム追加",
    sql: "ALTER TABLE comments ADD COLUMN IF NOT EXISTS seen_by TEXT[] NOT NULL DEFAULT '{}';",
  },
  {
    label: "comments Realtime 追加",
    sql: `DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname='supabase_realtime' AND tablename='comments') THEN
        ALTER PUBLICATION supabase_realtime ADD TABLE comments;
      END IF;
    END $$;`,
  },
  {
    label: "Storage バケット (visitor-images) 作成",
    sql: `INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
      VALUES ('visitor-images', 'visitor-images', true, 5242880, ARRAY['image/jpeg','image/png','image/gif','image/webp','image/heic'])
      ON CONFLICT (id) DO NOTHING;`,
  },
  {
    label: "Storage 読み取りポリシー",
    sql: `DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='objects' AND schemaname='storage' AND policyname='public_read_visitor_images') THEN
        CREATE POLICY "public_read_visitor_images" ON storage.objects FOR SELECT USING (bucket_id = 'visitor-images');
      END IF;
    END $$;`,
  },
  {
    label: "Storage アップロードポリシー",
    sql: `DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='objects' AND schemaname='storage' AND policyname='public_upload_visitor_images') THEN
        CREATE POLICY "public_upload_visitor_images" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'visitor-images');
      END IF;
    END $$;`,
  },
  {
    label: "Storage 削除ポリシー",
    sql: `DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='objects' AND schemaname='storage' AND policyname='public_delete_visitor_images') THEN
        CREATE POLICY "public_delete_visitor_images" ON storage.objects FOR DELETE USING (bucket_id = 'visitor-images');
      END IF;
    END $$;`,
  },
];

export async function POST(req: NextRequest) {
  const { pat } = await req.json();

  if (!pat?.trim()) {
    return NextResponse.json(
      { error: "Personal Access Token が必要です" },
      { status: 400 }
    );
  }

  const steps: { label: string; ok: boolean; detail?: string }[] = [];

  for (const step of SQL_STEPS) {
    const result = await runSQL(pat.trim(), step.sql);
    steps.push({
      label: step.label,
      ok: result.ok,
      detail: result.ok
        ? undefined
        : JSON.stringify(result.data).slice(0, 150),
    });
  }

  const allOk = steps.every((s) => s.ok);
  return NextResponse.json({ ok: allOk, steps });
}
